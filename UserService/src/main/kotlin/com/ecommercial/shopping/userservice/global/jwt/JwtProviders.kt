...
import com.ecommercial.shopping.userservice.global.exception.JWTErrorEnum
...
class JwtProviders(
    ...
    fun createToken(user: User): JwtTokenResponse {
        val claims = Jwts.claims()

        claims[jwtProperties.headers] = user.email
        val now = Date().time
        ...
        val accessToken = Jwts.builder()
            ...
            .claim("account-type", "User")
            .claim("grade", user.grade.gradeName)
    ...
    fun validateToken(token: String): Boolean {
        try {
            userRedisService.addBlackList(token, getExpirationFromToken(token))
